...
class FilesystemTenancyBootstrapper implements TenancyBootstrapper
{
    ...
    public function bootstrap(Tenant $tenant)
    {
        ...
        if ($this->app['config']['tenancy.filesystem.asset_helper_tenancy'] ?? true) {
            if ($this->originalPaths['asset_url']) {
                ...
                $this->app['url']->setAssetRoot($this->app['url']->route('tenant.asset', ['disk' => 'public', 'path' => '']));
            }
        }
        ...
        foreach ($this->app['config']['tenancy.filesystem.disks'] as $disk) {
            ...
            $localPath = str_replace('/app', '', $finalPrefix) . '/framework/cache';
        ...
        $this->app['config']['filesystems.disks.public.bucket'] = $tenant->bucketName;
        $this->app['config']['filesystems.disks.attachments.bucket'] = $tenant->bucketName;
    }
    ...
    public function revert()
    {
        ...
        $this->app['config']['filesystems.disks.public.bucket'] = env('AWS_BUCKET');
        $this->app['config']['filesystems.disks.attachments.bucket'] = env('AWS_BUCKET');
    }
}

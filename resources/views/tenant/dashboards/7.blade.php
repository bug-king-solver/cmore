            ...
            <div class="mt-10 pagebreak print:mt-20">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-10 print:-mt-10 nonavoid">
                    ...
                    @if(isset($charts['water_consumption']['value']))
                        @php
                            $subpoint = json_encode([ [ 'color' => 'bg-[#006CB7]', 'text' => __('Consumed water ('.$charts['water_consumption']['unit'].')') ]]);
                            $text = json_encode([__('Annual Water Consumption')]);
                        @endphp
                        <x-cards.card-dashboard-version1 text="{{ $text }}" subpoint="{{ $subpoint }}" class="relative">
                            <p class="text-esg8 font-medium text-5xl absolute w-full  text-center mt-40 -ml-4">
                            <x-number :value="$charts['water_consumption']['value']" />
                            <span class="text-esg8 font-normal text-base">{{ $charts['water_consumption']['unit'] }}</span>
                            </p>
                            @include('icons.dashboard.liquid')
                        </x-cards.card-dashboard-version1>
                    @endif

                    @if(isset($charts['waste_generated']['value']))
                        @php
                            $subpoint = json_encode([ ['text' => __('Total waste ('.$charts['waste_generated']['unit'].')') ]]);
                    ...
                    @if(isset($charts['recycled_waste']['value']))
                        @php
                            $subpoint = json_encode([ ['text' => __('Total waste ('.$charts['waste_generated']['unit'].')') ]]);
                            $text = json_encode([__('Amount of Treated or Recycled Waste')]);
                        @endphp
                        <x-cards.card-dashboard-version1 text="{{ $text }}" subpoint="{{ $subpoint }}">
                            <x-charts.icon-number value="{{ $charts['recycled_waste']['value'] }}" unit="{{ $charts['recycled_waste']['unit'] }}">
            ...
            <div class="mt-10 pagebreak print:mt-20">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-10 print:-mt-10 nonavoid">
                    ...
                    @if(isset($charts['work_days_lost']['value']))
                        ...
                        @endphp
                        <x-cards.card-dashboard-version1 text="{{ $text }}">
                            <x-charts.icon-number value="{{ $charts['work_days_lost']['value'] }}" unit="{{ $charts['work_days_lost']['unit'] }}">

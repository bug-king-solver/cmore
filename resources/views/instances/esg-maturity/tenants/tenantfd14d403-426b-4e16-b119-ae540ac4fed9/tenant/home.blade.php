...
<div class="px-4 md:px-0">
    <div class="bg-esg27 h-screen min-h-[1800px] md:min-h-0 md:max-h-[750px] flex left-[50%] -ml-[50vw] w-screen relative items-center justify-center mb-40 md:mb-0">
        <div class="w-full mx-auto max-w-7xl pt-8 px-4 sm:px-6 lg:px-8 absolute z-90">
            <div class="grid grid-cols-1 md:grid-cols-2 font-encodesans text-esg8 gap-14">
                <div class="mt-24">
                    ...
                    <p class="text-base font-normal text-justify text-esg8">{{ __('Here you will find everything you need to be successful on this journey to help companies turn sustainability into business.') }}</p>

                    <p class="text-base font-bold text-justify text-esg8 mt-8">{{ __('Let\'s work together for a sustainable future!') }}</p>
                    ...
                    @endphp
                    <button class="bg-esg5 w-60 h-11 rounded-lg font-bold text-base mt-11 uppercase text-esg27" @click="window.location.href = '{!! $questionnaireUrl !!}'"> {{ $btnText }} </button>
                </div>
        ...
        <div class="bg-esg6 h-[50vw] min-h-[100px] md:min-h-0 md:max-h-56 flex left-[50%] -ml-[50vw] w-screen relative items-center justify-center">
                <div class="grid items-center text-center font-encodesans text-esg27 gap-14">
                        <p class="text-4xl font-bold">{{__('Primeiro mercado regulado de negociação de ações tokenizadas do Brasil.')}}</p>
                </div>
        </div>

    <div class="font-encodesans mt-20 mb-20">
        <div class="max-w-5xl mx-auto">
            ...
            <p class="text-base font-normal text-esg8 text-center mt-5"> {{ __('Meet and explore a world of features designed to make it easier to collect and analyse data.') }} </p>

            <div class="grid grid-cols-1 md:grid-cols-2 md:gap-9 mt-10">

                @can('questionnaires.view')
                <div x-data="{ open: false }" @mouseleave="open = false" @mouseover="open = true" class="bg-esg4 border border-esg4 duration-500 hover:bg-esg6/[0.07] rounded-2xl p-8 cursor-pointer flex gap-5">
                    <a href="{{ route('tenant.questionnaires.panel') }}" class="flex gap-5">
                        <div class="grid justify-items-center">
                            @include('icons.home.1', ['color'=>'#153A5B'])
                        </div>

                        <div class="font-normal text-esg8 text-sm relative">
                            <p class="text-esg6 text-2xl font-semibold "> {{ __('Questionnaires') }} </p>
                            <p class="mt-3 font-normal text-base"> {{ __('An easy to navigate questionnaire based on the company\'s traits.') }} </p>
                ...
                <div x-data="{ open: false }" @mouseleave="open = false" @mouseover="open = true" class="bg-esg4 border border-esg4 duration-500 hover:bg-esg6/[0.07] rounded-2xl p-8 cursor-pointer flex gap-5">
                    <a href="{{ route('tenant.dashboard') }}" class="flex gap-5">
                        <div class="grid justify-items-center">
                            @include('icons.home.2', ['color'=>'#153A5B'])
                        </div>

                        <div class="font-normal text-esg8 text-sm relative">
                            <p class="text-esg6 text-2xl font-semibold "> {{ __('Dashboard') }} </p>
                            <p class="mt-3 font-normal text-base"> {{ __('A visual presentation of the analyzed data collected on the questionnaire.') }} </p>
                ...
                <div x-data="{ open: false }" @mouseleave="open = false" @mouseover="open = true" class="hidden bg-esg4 border border-esg4 duration-500 hover:bg-esg6/[0.07] rounded-2xl p-8 cursor-pointer flex gap-5">
                    <a href="{{ route('tenant.questionnaires.panel') }}" class="flex gap-5">
                        <div class="grid justify-items-center">
                            @include('icons.home.3', ['color'=>'#153A5B'])
                        </div>

                        <div class="font-normal text-esg8 text-sm relative">
                            <p class="text-esg6 text-2xl font-semibold "> {{ __('Document Analysis') }} </p>
                            <p class="mt-3 font-normal text-base"> {{ __('A tool to validate company’s compliance with ESG rules and criteria.') }} </p>
                ...
                <div x-data="{ open: false }" @mouseleave="open = false" @mouseover="open = true" class="hidden bg-esg4 border border-esg4 duration-500 hover:bg-esg6/[0.07] rounded-2xl p-8 cursor-pointer flex gap-5">
                    <a href="{{ route('tenant.questionnaires.panel') }}" class="flex gap-5">
                        <div class="grid justify-items-center">
                            @include('icons.home.4', ['color'=>'#153A5B'])
                        </div>

                        <div class="font-normal text-esg8 text-sm relative">
                            <p class="text-esg6 text-2xl font-semibold "> {{ __('Reputational Analysis') }} </p>
                            <p class="mt-3 font-normal text-base"> {{ __('An overview of a company’s perception and reputation.') }} </p>
                ...
                <div x-data="{ open: false }" @mouseleave="open = false" @mouseover="open = true" class="bg-esg4 border border-esg4 duration-500 hover:bg-esg6/[0.07] rounded-2xl p-8 cursor-pointer flex gap-5">
                    <a href="{{ route('tenant.targets.index') }}" class="flex gap-5">
                        <div class="grid justify-items-center">
                            @include('icons.home.5', ['color'=>'#153A5B'])
                        </div>

                        <div class="font-normal text-esg8 text-sm relative">
                            <p class="text-esg6 text-2xl font-semibold "> {{ __('Targets') }} </p>
                            <p class="mt-3 font-normal text-base"> {{ __('Create goals and track the progress of an indicator.') }} </p>
                ...
                <div x-data="{ open: false }" @mouseleave="open = false" @mouseover="open = true" class="bg-esg4 border border-esg4 duration-500 hover:bg-esg6/[0.07] rounded-2xl p-8 cursor-pointer flex gap-5">
                    <a href="{{ route('tenant.library.index') }}" class="flex gap-5">
                        <div class="grid justify-items-center">
                            @include('icons.home.6', ['color'=>'#153A5B'])
                        </div>

                        <div class="font-normal text-esg8 text-sm relative">
                            <p class="text-esg6 text-2xl font-semibold "> {{ __('Library') }} </p>
                            <p class="mt-3 font-normal text-base"> {{ __('Information about the software, ESG frameworks, and much more!') }} </p>

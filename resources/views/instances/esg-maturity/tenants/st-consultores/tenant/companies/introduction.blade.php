<div class="shadow rounded border-b-4 border-esg5 px-4 py-8 ">
    <div class="grid justify-items-center -mt-16">
        @include('icons.st_consultants.identify', ['fill' => color(5)])
    </div>

    <p class="mt-4 text-lg font-bold text-center"> {{ __('Identify the company') }} </p>

    <p class="text-base text-center mt-4 text-esg8">
        {{ __('The first step is to fill out simple information about your company that will allow us to identify the next steps in your journey.') }}
    </p>
</div>
...
@if (request()->routeIs('tenant.companies.form'))
    <div class="row mt-10 text-sm">
        <p class="text-red-500">
            ...
            <a href="{{ tenant()->getTermsConditionsUrl(session()->get('locale')) }}"
                class="underline">{{ __('See the full text of the terms and conditions here.') }}</a>
        </p>
    </div>
@endif

<div>
    <x-slot name="header">
        <x-header title="{{ $documentFolder->name }}" data-test="library-header" click="{{ route('tenant.library.index') }}">
            <x-slot name="left">
            </x-slot>
            <x-slot name="right">
                @if(auth()->user()->type == 'internal' && auth()->user()->can('library.create'))
                    @php
                        $data = json_encode(["parent" => $documentFolder->id]);
                        $attachdata = json_encode(["model" => null, "modelType" => null, "documentFolder" => $documentFolder->id]);
                    @endphp

                    <div class="flex items-center justify-end space-x-2">
                        @if($allowToCreateSubFolder)
                        <x-dropdown.library>
                            <li>
                                <x-buttons.btn-text modal="documents.folder.modals.form" class="text-gray-700 w-full py-2 px-2 hover:bg-gray-100 inline-flex items-center" :data="$data">
                                    @include('icons/library/plus-folder', ['class' => 'mr-2']) {{ __('New Folder') }}
                                </x-buttons.btn-text>
                            </li>
                            <li>
                                <x-buttons.btn-text modal="modals.attachments" :data="$attachdata" class="text-gray-700 w-full py-2 px-2 hover:bg-gray-100 inline-flex items-center">
                                    @include('icons/library/upload', ['class' => 'mr-2']) {{ __('Upload') }}
                                </x-buttons.btn-text>
                            </li>
                        </x-dropdown.library>
                        @else
                            <x-buttons.btn-icon-text modal="modals.attachments" :data="$attachdata">
    ...
    <div class="px-4 lg:px-0 mt-8">
        <div class="mx-auto max-w-7xl sm:px-6 px-4 lg:px-0 leading-normal mb-14">
            @if($allowToCreateSubFolder)
            <p class="font-semibold text-esg5 text-sm mb-4 pl-6 mt-14">{{ __('Sub Folders') }}</p>
            <div class="overflow-hidden overflow-x-auto">
                <x-tables.bordered.table aria-describedby="{{ __('Sub Folders') }}">
                    <x-slot name="thead">
                    ...
                    @if($documentFolder->accessibleChilds()->count() > 0)
                    @foreach($documentFolder->accessibleChilds()->get() as $folder)
                        @php
                            $fullSlug = $documentFolder->fullSlug.'/'.$folder->slug;
                        @endphp
                        <x-tables.bordered.tr class="text-esg8 hover:bg-gray-100">
                            <x-tables.bordered.td>
                                <a class="flex items-center hover:text-esg6" href="{{ route('tenant.library.folder.show', ['slug' => $fullSlug]) }}">
                                    <div class="mr-3">
            ...
            <p class="font-semibold text-esg5 text-sm mb-4 pl-6 mt-14">{{ __('All Files') }}</p>
                <div class="overflow-hidden overflow-x-auto">
                    <x-tables.bordered.table aria-describedby="{{ $documentFolder->name }}">
                        <x-slot name="thead">
                                    ...
                                    <div class="flex w-full items-center justify-end space-x-4">
                                        @can('library.view')
                                            @if(tenant()->onActiveSubscription)
                                                <x-buttons.a-icon class="inline py-1.5 px-2 text-sm" href="{{ privateAssetDownload($document->id, 'attachments') }}">

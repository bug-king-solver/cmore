                    ...
                    @if (isset($value[$question_option->option->id]) && $value[$question_option->option->id])
                        <p class="p-2">
                            ...
                            @if (
                                ...
                                @if ($customDataType === 'short')
                                    <input type="text" class="ml-4 rounded-lg w-96"
                                        wire:change="save({{ $question_option->id }});"
                                        wire:model.lazy="customData.{{ $question_option->option->id }}">
                                @elseif ($customDataType === 'long')
                                    <br>
                                    <textarea
                                        {{ $questionnaire->isSubmitted() ||$this->answer->validation ||auth()->user()->cannot('questionnaires.answer')? 'disabled': '' }}
                                        wire:change="save({{ $question_option->id }})" wire:model.lazy="customData.{{ $question_option->option->id }}"
                                        rows="3" class="my-2 focus:border-esg6 w-2/3 rounded-lg focus:ring-0"></textarea>
                                @endif
                            @endif
                        </p>
                    @endif
